...
UniFly::UniFly()
    : m_aiControlled("unifly/ai_controlled", ReadWrite),
      ...
      m_altitudeTemperatureEffect(
          "sim/weather/aircraft/altimeter_temperature_error", ReadOnly),
...
void UniFly::Initialize() {
  ...
  XPLMRegisterCommandHandler(CommandAircraftLabelShow,
                             HandleCommandAircraftLabelShow, 1, this);
  XPLMRegisterCommandHandler(CommandAircraftLabelType,
                             HandleCommandAircraftLabelType, 1, this);
  XPLMRegisterCommandHandler(CommandAircraftLabelRange,
                             HandleCommandAircraftLabelRange, 1, this);
  XPLMRegisterCommandHandler(CommandAircraftLabelColour,
                             HandleCommandAircraftLabelColour, 1, this);
  XPLMRegisterCommandHandler(CommandAircraftLabelVisibilityCutoff,
                             HandleCommandAircraftLabelVisibilityCutoff, 1,
                             this);
...
int CBIntPrefsFunc(const char *, [[maybe_unused]] const char *item,
  ...
  if (!strcmp(item, XPMP_CFG_ITM_HANDLE_DUP_ID))
    // If an XPMPPlaneId is not unique, throw an error rather than blindly
    // choosing a new one
    return false;
...
bool UniFly::InitializeXPMP() {
  ...
  auto err =
      XPMPMultiplayerInit("UniFly", pathResources.c_str(), &CBIntPrefsFunc);
...
float UniFly::MainFlightLoop(float inElapsedSinceLastCall, float, int inCounter,
                             void *ref) {
  auto *instance = static_cast<UniFly *>(ref);
  if (instance) {
    FlushMsgs();
    ...
    unifly::schema::v1::EventFrame *event_frame =
        event_frame_message.mutable_event_frame();
    ...
    unifly::schema::v1::LocalReadFrequent *read_frequent =
        read_frequent_message.mutable_local_read_frequent();
    ...
    read_frequent->set_speed_ground(instance->m_groundSpeed *
                                    1.94384); // meters per second to knots
    ...
    if (inCounter % (90 * 2) <= 1) {
      ...
      unifly::schema::v1::LocalReadInfrequent *read_infrequent =
          read_infrequent_message.mutable_local_read_infrequent();
  ...
  try {
    ...
    while (m_keepSocketAlive.load()) {
      ...
      if (ec == asio::error::would_block) {
        ...
        // We don't want accept() blocking when m_keepSocketAlive has been set
        // to false
...
void UniFly::ProcessPacket(const unifly::schema::v1::XPlaneMessage msg) {
  ...
  case unifly::schema::v1::XPlaneMessage::kLocalTeleport: {
    QueueCallback([msg = std::move(msg), this]() mutable {
      ...
      XPLMWorldToLocal(teleport.lat(), teleport.lon(),
                       teleport.alt_msl() / 3.2808399, // Feet to meters
                       &pos_x, &pos_y, &pos_z);
...
void UniFly::DeleteAllAircraft() { m_aircraftManager->DespawnAll(); }
...
bool UniFly::IsXplane12() { return XPlaneVersion >= 120000; }
} // namespace unifly
