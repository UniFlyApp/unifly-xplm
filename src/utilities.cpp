...
const char *LogGetString(const char *szPath, int ln, const char *szFunc,
  ...
  if (args) {
    vsnprintf(&aszMsg[strlen(aszMsg)],
              sizeof(aszMsg) - strlen(aszMsg) -
                  1, // we save one char for the CR
              szMsg, args);
  }
...
void FlushMsgs() {
  // only if the flag says there's something waiting will we do the more
  // expensive lock operation
  if (!fMsgsEmpty.test_and_set()) {
    LOG_ASSERT(unifly::global.IsXPThread());

    std::lock_guard<std::mutex> lock(
        mtxMsgs);            // access to queue guarded by lock
...
void PushMsg(const std::string &sMsg) {
  // If we are in XP's main thread we can just write...but only after waiting
  // msgs
...
const char *UniFlyError::what() const noexcept { return msg.c_str(); }
