...
bool send_message(tcp::socket &socket,
                  const unifly::schema::v1::XPLMMessage &message) {
  ...
  google::protobuf::io::CodedOutputStream::WriteLittleEndian32ToArray(
      message_length, buffer.data());

  // Serialize the message directly into the buffer after the prefix
  if (!message.SerializeToArray(buffer.data() + prefix_length,
                                message_length)) {
...
bool recv_message(tcp::socket &socket,
  ...
  size_t prefix_read =
      asio::read(socket, asio::buffer(prefix_buf, sizeof(prefix_buf)), ec);
  if (ec || prefix_read != sizeof(prefix_buf)) {
    LOG_MSG("recv_message: Failed to read length prefix %s",
            ec.message().c_str());
    return false;
  }
  ...
  size_t body_read =
      asio::read(socket, asio::buffer(buffer.data(), message_length), ec);
  if (ec || body_read != message_length) {
    LOG_MSG("recv_message: Failed to read message body %s",
            ec.message().c_str());
    return false;
  }
